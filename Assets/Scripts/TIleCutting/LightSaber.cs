...
public class Cutter : MonoBehaviour
{
    ...
    public List<GameObject> cutParts;
    ...
    {
        if (other.GetComponent<ActualTile>())
        {
            ...
            cutParts.Clear();
            cutParts.Add(slices[0]);
            cutParts.Add(slices[1]);
            Rigidbody rigidbody = slices[0].GetComponent<Rigidbody>();
            Vector3 newNormal = transformedNormal + Vector3.up * _forceAppliedToCut;
            rigidbody.AddForce(newNormal, ForceMode.Impulse);
        }
        // Rigidbody rigidbody2 = slices[1].GetComponent<Rigidbody>();
        // rigidbody2.useGravity = false;
        // rigidbody2.constraints = RigidbodyConstraints.FreezeAll;
    }

    public void MoveTheCutter(Transform position1, Transform position2)
    {
        this.transform.localPosition = position1.localPosition;
        this.transform.localRotation = Quaternion.Euler(0, 0, 0);
        this.transform.DOLocalMove(new Vector3(0, 0, -0.4f), 1f);
        StartCoroutine(snapBackTile());
    }

    IEnumerator snapBackTile()
    {
        yield return new WaitForSeconds(1.1f);
        tileCasting.SnapBackCuttedTile();
    }
}
