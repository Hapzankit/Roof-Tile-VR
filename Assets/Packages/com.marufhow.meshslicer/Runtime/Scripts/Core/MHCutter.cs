...
namespace com.marufhow.meshslicer.core
{
    public class MHCutter : MonoBehaviour
    {
        ...
        public void Cut(GameObject cutObject, float percentage)
        {
            // Calculate the center of the object using its bounds
            Vector3 cutPoint = cutObject.GetComponent<Renderer>().bounds.center;

            // Calculate the size of the object (width, height, depth)
            Vector3 objectSize = cutObject.GetComponent<Renderer>().bounds.size;

            // Calculate the offset based on the percentage, applying it to the X-axis (right/left)
            // If percentage is 30, it will move 30% to the right of the center, if -30, it will move 30% to the left
            float offsetX = objectSize.x * (percentage / 100f);

            // Apply the offset to the center position (moving along the X-axis)
            cutPoint += new Vector3(offsetX, 0, 0);

            Vector3 cutNormal = -Vector3.right;

            // No need for InverseTransformDirection and InverseTransformPoint anymore
            Plane plane = new Plane(cutNormal, cutPoint);

            print(cutPoint + " Point" + cutNormal + " Plane");
            ...
            {
                var subMeshTriangles = _mesh.GetTriangles(i);
